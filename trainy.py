...
def handle_fallback_response(context):
    ...
    user_response = st.text_input(context.user_name)
    if user_response.lower() in ["yes", "yeah", "ok", "sure"]:
    ...
    elif user_response.lower() in ["no", "nope"]:
...
def handle_user_input():
    ...
    if user_input.lower() in ["quit", "exit"]:
        st.success("Mich Bot: Goodbye! Chatbot is now exiting.")
        st.stop()
    ...
    st.markdown(
        f"<div style='margin-bottom: 5px;'><span style='color: #0366d6; font-weight: bold;'>You:</span> {user_input}</div>",
        unsafe_allow_html=True)
    st.markdown(response, unsafe_allow_html=True)


# Create a Streamlit app
st.title("Mich Bot Chat Interface")
context = ChatContext()
handle_user_input()
